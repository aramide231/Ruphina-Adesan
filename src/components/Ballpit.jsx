...
class Z extends d {
  ...
  #S() {
    ...
    const lightColor = Array.isArray(this.config.colors) && this.config.colors.length
      ? this.config.colors[0]
      : 0xffffff;
    this.light = new u(lightColor || 0xffffff, this.config.lightIntensity);
    this.add(this.light);
  }
  setColors(e) {
    const palette =
      Array.isArray(e) && e.length > 1 && e.some(col => col !== 0)
        ? e
        : [0xf4c95f, 0xe8a0bf, 0x7eb8a2, 0xf7f1e8, 0xd4a574];
...
function createBallpit(e, t = {}) {
  ...
  function initialize(e) {
    ...
    i.scene.add(s);
    s.update({ delta: 0.016, elapsed: 0 });
  }
